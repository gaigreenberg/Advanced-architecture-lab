asm_cmd(ADD, 6, 0, 1, 1);      // 0: R6 = 1 constant
...
asm_cmd(ADD, 7, 0, 0, 0);      // 10: R7 = 0 (result)
...
asm_cmd(ADD, 7, 7, 2, 0);      // 14: R7 = R7 + R2 (result += X )
asm_cmd(SUB, 3, 3, 1, 1);      // 15: R3 -= 1 (Y--)
asm_cmd(LSF, 7, 7, 1, 1);      // 16: R7 = R7 << 1 ( result * 2 )
asm_cmd(RSF, 3, 3, 1, 1);      // 17: R3 = R3 >> 1 ( Y / 2 )
asm_cmd(JNE, 0, 0, 3, 12);     // 18: if( R3 != 0 ) jump to loop 12
asm_cmd(JNE, 0, 4, 6, 21);     // 19: if( R4 != R6 ) jump to storing ( else means the output should be negative)
asm_cmd(SUB, 7, 0, 3, 0);      // 20: R7 = - R7
asm_cmd(ST,  0, 7, 1, 1002);   // 21: store R7 (result) ro mem[1002]
